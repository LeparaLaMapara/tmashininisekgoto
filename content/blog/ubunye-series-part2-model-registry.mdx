...
seoTitle: "Ubunye Engine Part 2: The Model Registry"
date: 2026-03-06
...
*Part 2 of 6 in the Ubunye Engine series.*
...
## Why This Was Not In That Version

It is a migration. Every `transform()` function at the time received a `pyspark.sql.DataFrame`. Every `Reader.read()` returned one. Adding `DataFramePort` as the official interface needed a release with a clear migration path.
...
**Update, July 2026.** Version 0.5.0 shipped steps 1 to 3 exactly as written above. `DataFramePort` is now a real Protocol in the engine, Spark DataFrames satisfy it without modification, and `PandasDataFrameAdapter` exists for tests and local work. Step 4, a full pandas execution backend, is tracked as an open issue on the repo. It is a good feeling when a design section becomes a changelog entry. [Part 6](/blog/ubunye-series-part6-how-to-use-it) shows the engine as it stands now.
