...
![Two boards compared. On the left, infrastructure built for applications: Kubernetes, Jenkins and CI pipelines, SQL Server on premises, PostgreSQL databases, Windows virtual machines, some work on somebody's laptop and databases with no obvious owner. On the right, where the data science work was going: large volumes, reprocessed history and model training. The workload and the infrastructure did not share a shape.](/posts/ds_estate.png)
...
![A route with seven stops in order: make the work visible, write everything down, play the long game, pay the infrastructure bill, teach the team you have, patterns not heroics, and learn to let go, ending with systems that outlive people.](/posts/ds_order.png)
...
![Three small charts of backlog over time. When data arrives faster than it is processed, the backlog grows. At the same rate in and out, it stops growing but never shrinks. Only when processing is faster than arrival does the backlog clear.](/posts/ds_backlog.png)
...
![Three requirements, work through the backlog, keep up with what arrives daily and trust the data before anything downstream uses it, leading to six controls: reliable ingestion and ETL, reconciliation to prove a number, quality checks, monitoring and alerts, lineage, and safe reprocessing of history.](/posts/ds_recovery.png)
...
![Two parallel tracks. Track one, today: get enough data onto the platform, processing, and supporting the business right now. Track two, the capability: architecture, funding, contracts, access, support and adoption. Only track one is an emergency measure; only track two is a beautiful architecture and an angry business.](/posts/ds_two_tracks.png)
...
![A loop of six steps: work it out, prove it, write it down, teach it, somebody else uses it, and nobody remembers it was special. The goal was never to be the expert everyone comes back to.](/posts/ds_knowledge_loop.png)
...
![A stack of five layers from the bottom up: platform for compute, storage and catalog; data engineering for ingestion, ETL and quality; data science for experiments, models and deployment; practices for documentation, standards, testing and CI/CD; and people for skills, trust and ownership. Take any layer out and the ones above it wobble.](/posts/ds_capability_stack.png)
...
![On the left, a data science team that for a while also carries data engineering, platform enablement and everything nobody else picked up, which is necessary for a while and a terrible permanent arrangement. On the right, the work handed to the teams that should own it, giving data scientists their capacity back.](/posts/ds_hand_over.png)
...
![Before and after two years. Time to production went from unbounded to a new model shipping in days; work that piled up became capacity to work through it; onboarding by oral tradition became a documented path; inherited models nobody could explain became documented and governed; and a reactive team became a proactive one.](/posts/ds_two_years.png)
